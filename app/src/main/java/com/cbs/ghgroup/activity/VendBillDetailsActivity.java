...
import com.cbs.ghgroup.adapter.VendBillRegisterAdapter;
import com.cbs.ghgroup.databinding.ActivityVendBillDetailsBinding;
...
public class VendBillDetailsActivity extends AppCompatActivity {
    ActivityVendBillDetailsBinding vendBillDetailsBinding;
    Context mContext;
    RecyclerView rvVendBillReg;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        vendBillDetailsBinding = DataBindingUtil.setContentView(this,R.layout.activity_vend_bill_details);
        mContext = VendBillDetailsActivity.this;
    ...
    public void getVendBillRegister(){
        CustomProgressbar.showProgressBar(mContext, false);
        RetrofitClient.getClient().getBillRegister(fromdate,todate,bpcode,branch_code,"V").enqueue(new Callback<BillRegister>() {
            @Override
            public void onResponse(Call<BillRegister> call, Response<BillRegister> response) {
                ...
                if (response.code() == 200 && response.body() != null) {
                    ...
                    if (billRegister.getBillRegisterResult().getBillRegisterDetail().size() > 0) {
                        ...
                        rvVendBillReg = (RecyclerView) findViewById(R.id.rv_vendbill_reg_details);

                        LinearLayoutManager linearLayoutManager = new LinearLayoutManager(getApplicationContext());
                        rvVendBillReg.setLayoutManager(linearLayoutManager);
                        VendBillRegisterAdapter billRegisterAdapter = new VendBillRegisterAdapter(VendBillDetailsActivity.this, billRegisterDetail);
                        rvVendBillReg.setAdapter(billRegisterAdapter);
                        ...
                        for (int i = 0;i<billRegisterDetail.size();i++){
                            ...
                            billRegbAmt = billRegbAmt+totalamt;
                            DecimalFormat formatter = new DecimalFormat("##,##,##,##,###");
                            vendBillDetailsBinding.vendBillRegShowTotal.setText("Total:  " +formatter.format(billRegbAmt));
                        ...
                        try {
                            ...
                            for (int i = 0; i < billRegisterDetail.size(); i++) {
                                ...
                                fileName = billRegisterDetail.get(i).getBillAmt();
                                if (billRegisterDetail.get(i).getBranch() != "Total :") {
//                                    DecimalFormat formatter = new DecimalFormat("##,##,##,##,###");
                                    //paymentCommDetailsBinding.payCommShowTotal.setText("Total:  " +formatter.format(paybill));
                                    vendBillDetailsBinding.vendBillRegShowTotal.setText(fileName);
                                    ...
                                    DecimalFormat formatter = new DecimalFormat("##,##,##,##,###");
                                    //paymentCommDetailsBinding.payCommShowTotal.setText("Total:  " +formatter.format(paybill));
                                    vendBillDetailsBinding.vendBillRegShowTotal.setText("Total:  " + formatter.format(paybill));
                                }
                            }
        ...
        vendBillDetailsBinding.billRegExcelDownload.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                ...
                RetrofitClient.getClient1().downloadExcel(excelData).enqueue(new Callback<String>() {
                    ...
                    public void onResponse(Call<String> call, Response<String> response) {
                        ...
                        if (response.code() == 200 && response.body() != null) {
                            ...
                            Toast.makeText(VendBillDetailsActivity.this, "Unable to download excel", Toast.LENGTH_SHORT).show();
                        }
                    }
