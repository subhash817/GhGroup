...
public class VendGRAdapter extends RecyclerView.Adapter<VendGRAdapter.myViewHolder>{
    private List<CommunicationRegisterDetail> communicationRegisterDetails;
    Context context;

    public VendGRAdapter(Context context, List<CommunicationRegisterDetail> communicationRegisterDetails) {
    ...
    @NonNull
    @Override
    public VendGRAdapter.myViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        LayoutInflater layoutInflater = LayoutInflater.from(parent.getContext());
        View view = layoutInflater.inflate(R.layout.general_report_detail, parent, false);
        return new VendGRAdapter.myViewHolder(view);

    }

    @Override
    public void onBindViewHolder(@NonNull VendGRAdapter.myViewHolder holder, int position) {
        ...
        holder.tv_VendName.setText(communicationRegisterDetail.getCustomerName());
        ...
        if (communicationRegisterDetail.getCustomerName().equalsIgnoreCase("Total :")) {
            holder.tv_VendName.setVisibility(View.INVISIBLE);

            if (communicationRegisterDetail.getTotalAmt().equalsIgnoreCase(communicationRegisterDetail.getTotalAmt())){
                holder.tv_TotalAmts.setVisibility(View.INVISIBLE);
            }
        }
