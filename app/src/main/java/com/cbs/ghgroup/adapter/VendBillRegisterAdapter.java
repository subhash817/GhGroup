...
public class VendBillRegisterAdapter extends RecyclerView.Adapter<VendBillRegisterAdapter.myViewHolder>{
    private List<BillRegisterDetail> billRegisterDetails;
    Context context;

    public VendBillRegisterAdapter(Context context, List<BillRegisterDetail> billRegisterDetails){
        this.billRegisterDetails=billRegisterDetails;
        this.context=context;
    }
    @NonNull
    @Override
    public VendBillRegisterAdapter.myViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        LayoutInflater layoutInflater=LayoutInflater.from(parent.getContext());
        View view=layoutInflater.inflate(R.layout.bill_register_detail,parent,false);
        return new VendBillRegisterAdapter.myViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull VendBillRegisterAdapter.myViewHolder holder, int position) {
        ...
        holder.tv_VendorName.setText(billRegisterDetail.getBpName());

        if (billRegisterDetail.getBranch().equalsIgnoreCase("Total :")) {
            holder.tv_Branch.setVisibility(View.INVISIBLE);

            if (billRegisterDetail.getBillAmt().equalsIgnoreCase(billRegisterDetail.getBillAmt())){
